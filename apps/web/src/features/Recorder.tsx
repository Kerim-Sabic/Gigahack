...
export function Recorder({meeting,t,done}:{meeting:string;t:Labels;done:()=>void}){
 ...
 function failure(e:Error){const message=e.name==='NotAllowedError'?'Microphone access was denied. Allow microphone access in your browser.':e.name==='NotFoundError'?'No microphone was found. Connect a microphone and try again.':e.name==='NotReadableError'?'The microphone is busy or unavailable. Check the device and try again.':e.message;setError(tr(message));setState('error');control.current?.node.port.postMessage('pause');}
 ...
 async function stop(){const c=control.current;if(!c)return;setState('saving');try{
  await new Promise<void>((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error(tr('Microphone flush timed out.'))),5000);c.flushed=()=>{clearTimeout(timer);resolve();};c.node.port.postMessage('stop');});
  c.stream.getTracks().forEach(track=>{track.onended=null;track.stop();});await c.queue.finish();if(!c.queue.nextSequence)throw new Error(tr('No audio was recorded.'));
 ...
 return <section className="capture"><div><strong>{t.saved}: {saved.toFixed(1)} s</strong><p>{tr("Confirmed local storage")}</p>{state!=='idle'&&<><meter min={0} max={1} value={level} aria-label={tr("Microphone level")}/><span className="caption">{tr(level>.95?'Clipping':level<.01?'Silence':'Microphone active')}</span></>}</div>
 {state==='idle'?<button className="secondary" onClick={start}>{t.record}</button>:<><span role="status">{tr(state)}</span><button className="secondary" onClick={pause} disabled={state==='saving'||state==='error'}>{state==='paused'?t.resume:t.pause}</button><button onClick={stop} disabled={state==='saving'}>{t.stop}</button><button className="secondary" onClick={()=>{const c=control.current;c?.gaps.push({bookmark_after_sequence:c.queue.nextSequence-1,acknowledged_seconds:saved});}}>{tr("Bookmark")}</button></>}
