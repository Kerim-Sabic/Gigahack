...
export class CaptureQueue {
 ...
 enqueue(data:ArrayBuffer){const samples=data.byteLength/2;if(this.pendingSamples+samples>this.rate*60)throw new Error(tr('Recording stopped: more than 60 seconds could not be saved.'));this.pending.push({sequence:this.nextSequence++,data});this.pendingSamples+=samples;if(!this.error)void this.drain();}
 ...
 async finish(){await this.drain();if(this.error)throw this.error;if(this.pending.length)throw new Error(tr('Audio is not fully acknowledged.'));}
}
